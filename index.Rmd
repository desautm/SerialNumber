...
Basé sur [How many Commodore 64 computers were really sold?](https://www.pagetable.com/?p=547)

Pour ce faire, nous allons utiliser la base de données du site [C64 Preservation](https://c64preservation.com/dp.php?pg=registry). Ce site héberge une base de données contenant les numéros de séries de certains Commodore 64.

--

```{r echo = FALSE}
commodore64 <- tibble(
  année = c(rep(1982, 5), rep(1983, 3), rep(1984, 3), 1986, 1987, 1988, 1989, 1990),
  série = c("326298", "326298/A", "326298/B", "326298/C", "KU-14194HB", "250407/A", "250407/B","250407/C", "250425/-", "250425/A", "250425/B", "250466", "250469/3", "250469/4", "250469/A", "250469/B"),
  n = c(4, 21, 6, 6, 16, 7, 50, 18, 48, 9, 4, 18, 14, 29, 16, 17),
  maximum = c(NA, 325512, NA, NA, NA, 208282, 1152644, 1218502, 1273699, 500165, 536345, 438001, 444384, 1124586, 1994012, 2242493)
)
DT::datatable(commodore64,
              fillContainer = FALSE, options = list(pageLength = 6), rownames = FALSE)
```

---

Nous allons débuter par calculer une estimation du nombre maximal de Commodore 64 produits pour les lignes où nous avons un maximum. Nous utilisons l'estimateur $\hat{N}_4$.
...
commodore64 <- commodore64 %>%
  mutate(total = round((n+1)/n*maximum-1))

DT::datatable(commodore64, options = list(pageLength = 6), rownames = FALSE)
```
...
* Le 11 juillet 2008, un utilisateur de produits Apple a demandé aux propriétaires leur numéro de série IMEI. 
* Le IMEI (*The International Mobile Equipment Identity*) est un code unqiue de 15 chiffres assigné à tous les téléphones cellulaires
...
La compagnie Bloomberg héberge un modèle pour estimer la production du modèle 3 de Tesla: [Tesla Model 3 Tracker](https://www.bloomberg.com/graphics/2018-tesla-tracker/#).

* La compagnie utilise, entre autre, le numéro de série VIN du modèle 3 de Tesla.
...
g <- ggplot()+
  ...
  geom_point(data = PRODUCTION, mapping = aes(x = date, y = total_unites), color = "green", alpha = 0)+
  geom_line(data = PRODUCTION, mapping = aes(x = date, y = total_unites), color = "green", alpha = 0)+
